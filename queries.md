...
db.companies.find({ $and : [ {founded_year : { $gte: 2000 } } , { founded_year : { $lte:2005 }} ]}, {founded_year:1, name:1, _id:0} );
...
db.companies.find({$and : [ {number_of_employees: { $lt: 1000 }} , {founded_year: { $lt: 2005 }} ]}).sort({founded_year:1}).limit(10);
...
<!-- Your Code Goes Here -->
filter:         {$nor: [{ ipo : { $exists : false} } , { ipo : null }]}
project:        
sort:           {"ipo.valuation_amount":-1}
skip:   
limit:

db.companies.find({$nor: [{ ipo : { $exists : false} } , { ipo : null }]}).sort({"ipo.valuation_amount":-1});
...
db.companies.find( { $and: [ { founded_month : { $gte:4 }} , { founded_month : { $lte : 6} }] }).limit(1000);
